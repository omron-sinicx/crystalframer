...
void fused_dpa_sigmamat_fwd(
    ...
    __shared__ float _output[THREAD_NUM][SIGMAMAT_DIM+1];
    ...
    if (bij_ehk != NULL) {
        ...
        {
            long long int j = edge_ij_e[E+e];
            const float* val_k = &val_ihk[(j*H+h)*SIGMAMAT_DIM];
            const float* bij_k = &bij_ehk[(e*H+h)*SIGMAMAT_DIM];
