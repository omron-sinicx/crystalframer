...
def _scaled_dot_product_attention(
    ...
    q = q / math.sqrt(E)
    # (M, B, E) x (M, B, E) -> (M, B)
    if config.REPRODUCIBLITY_STATE>=2 and dropout_p == 0.0 and batch is batch_kv:
...
def compute_real_domain_attn_low_memory(alpha, pos, dist2_min, tvecs, batch, edges, proj, K:int, dist_max:float, wscale:float, frame_method:str, rvecs:Tensor=None, cutoff:float=None):
    # alpha: (nodes, heads)
    # dist2: (edge_num, R)
    # pe_wave: (edge_num, R, K)
    # where R = (1+2r)^3 and K is the PE dim.
...
def compute_frame_maximum_method_(q, k, v, alpha, pos, dist2_min, tvecs, batch, edges, proj, K:int, dist_max:float, wscale:float, frame_method:str, this_epoch:int=None, rvecs:Tensor=None, cutoff:float=None):
    # alpha: (nodes, heads)
    # dist2: (edge_num, R)
    # pe_wave: (edge_num, R, K)
    # where R = (1+2r)^3 and K is the PE dim.
...
def compute_frame_lattice_method_(q, k, v, alpha, pos, dist2_min, tvecs, batch, edges, proj, K:int, dist_max:float, wscale:float, frame_method:str, this_epoch:int=None, rvecs:Tensor=None, cutoff:float=None):
    # alpha: (nodes, heads)
    # dist2: (edge_num, R)
    # pe_wave: (edge_num, R, K)
    # where R = (1+2r)^3 and K is the PE dim.
...
def compute_frame_pca_method_(q, k, v, alpha, pos, dist2_min, tvecs, batch, edges, proj, K:int, dist_max:float, wscale:float, frame_method:str, this_epoch:int=None, rvecs:Tensor=None, cutoff:float=None):
    adap_cutoff_args = ()
    if cutoff is not None:
        rvlen = torch.norm(rvecs, 2, dim=-1)
        adap_cutoff_args = (rvlen, cutoff)
...
def compute_real_domain_framed_proj(alpha, pos, dist2_min, tvecs, batch, edges, proj, proj_angle1, proj_angle2, proj_angle3, frame_vec_1, frame_vec_2, frame_vec_3, K:int, dist_max:float, wscale:float, dim_angle_enc:int, value_pe_angle_scale:float, cos_abs: int, length_rbf_mul:float, angle_rbf_mul:float, rvecs:Tensor=None, cutoff:float=None):
    adap_cutoff_args = ()
    if cutoff is not None:
        rvlen = torch.norm(rvecs, 2, dim=-1)
        adap_cutoff_args = (rvlen, cutoff)
    ...
    out = RealEncodingWithFramedProjFuncCUDA.apply(
        alpha, pos, dist2_min, tvecs, batch, edges, K, dist_max, wscale, proj_, proj_angle1, proj_angle2, proj_angle3, frame_vec_1, frame_vec_2, frame_vec_3,
...
def compute_real_domain_attn_frame_proj(
    ...
    if sigma_mat is not None:
        ...
        sigma_mat_fused = torch.zeros((sigma_mat_fused_tmp.shape[0], sigma_mat_fused_tmp.shape[1], 3, 3), device=sigma_mat_fused_tmp.device)
        ...
        if params.symm_break_noise > 0:
            sigma_mat_max = torch.zeros((sigma_mat_fused_tmp.shape[0], sigma_mat_fused_tmp.shape[1], 3, 3), device=sigma_mat_fused_tmp.device)
...
def crystalformer_multi_head_attention_forward_cuda(
    ...
    #
    attn_output, attn_output_weights, sizes = _scaled_dot_product_attention(q, k, v, batch_q, batch_kv, edges, attn_weights, values, dropout_p)
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, 
        ...
        if params.domain in ["real", "multihead"]:
            ...
            if params.domain == "real":
                self.proj_angle1 = Parameter(torch.empty(cond, head, self.head_dim, params.value_pe_angle_real, **factory_kwargs)) \
                    if params.value_pe_angle_real > 0 else None
                self.proj_angle2 = Parameter(torch.empty(cond, head, self.head_dim, params.value_pe_angle_real, **factory_kwargs)) \
                    if params.value_pe_angle_real > 0 else None
                self.proj_angle3 = Parameter(torch.empty(cond, head, self.head_dim, params.value_pe_angle_real, **factory_kwargs)) \
                    if params.value_pe_angle_real > 0 else None
            else:
