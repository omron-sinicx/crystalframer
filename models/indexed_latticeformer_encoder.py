...
class IndexedLatticeformerEncoder(Module):
    ...
    def forward(self, src: Tensor, pos: Tensor, batch: Tensor, trans: Tensor, sizes: Tensor, onehot: Tensor=None, this_epoch:int = None) -> Tensor:
...
class CrystalformerEncoderLayerCUDA(Module):
    ...
    def forward(self, src: Tensor, batch: Tensor, edges: Tensor,
        ...
        x = src
        if self.norm_first:
            x = x + self._sa_block(self.norm1(x), batch, edges, pos_ij, dist2_min, trans, kr_base, rvecs, vcell, onehot, this_epoch)
            x = x + self._ff_block(self.norm2(x))
        else:
            x = self.norm1(x + self._sa_block(x, batch, edges, pos_ij, dist2_min, trans, kr_base, rvecs, vcell, onehot, this_epoch))
            x = self.norm2(x + self._ff_block(x))
    ...
    def _sa_block(self, x: Tensor, batch: Tensor, edges: Tensor,
        ...
        kr_base: Tensor=None, rvecs: Tensor=None, vcell: Tensor=None,
        onehot:Tensor=None, this_epoch:int=None) -> Tensor:
        x = self.self_attn(x, x, x, batch, batch, edges, pos_ij, dist2_min, trans, kr_base, rvecs, vcell, onehot, need_weights=False, this_epoch=this_epoch)[0]
        return self.dropout1(x)
