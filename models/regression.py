...
class RegressionModel(LightningModule):
    def __init__(self, params, train_loader, val_loader):
        ...
        self.save_for_ckpt = self.params.save_path + "/" + self.params.experiment_name + "/version_0/model_checkpoint"

        if self.params.value_pe_angle_real == 0:
            self.params.frame_method = "no"

        print("Frame method: " + str(self.params.frame_method))
    ...
    def training_step(self, batch, batch_idx):
        ...
        # To avoid a program stop due to an out-of-memory exception,
        # ignore exceptions during training.
        try:
            ...
            print("An exception was occurred during a training step. We skipped this step. If this happens frequently, consider reducing the batch size.")
            print(e)
            output = None
    ...
    def on_train_epoch_end(self):
        ...
        if self.this_epoch%100 == 0 and self.this_epoch != 0:
            import glob
            import shutil
            ckpt_path = glob.glob(self.save_for_ckpt + "/epoch*.ckpt")
            shutil.copy(ckpt_path[0], self.save_for_ckpt + "/best_epoch_" + str(self.this_epoch) + ".ckpt")
    ...
    def forward(self, x):
        if self.use_average_model and self.swa_model is not None:
            return self.swa_model(x, this_epoch=self.this_epoch)
        return self.model(x, this_epoch=self.this_epoch)
