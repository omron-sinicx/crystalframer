    ...
    def forward(self, data, this_epoch=None):
        ...
        if not getattr(self.params, 'ddp', False) and device_count > 1 and x.is_cuda and batch_size > 1:
            ...
            if self.encoder.layers[0].self_attn.gauss_scale[0] == 0:
                ...
                with torch.no_grad():
                    x0 = self.encoder(x, pos, batch, trans, sizes, onehot_x, this_epoch)
            ...
            if True:
                ...
                sizes_ = [inputs[dev][-3]for dev in range(n)]
            ...
            x = self.encoder(x, pos, batch, trans, sizes, this_epoch = this_epoch)
